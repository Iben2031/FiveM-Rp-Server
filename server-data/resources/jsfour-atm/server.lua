...
AddEventHandler('jsfour-atm:insert', function(amount)
	...
	if amount > xPlayer.getMoney() then
		...
		TriggerClientEvent('esx:showNotification', _source, 'you deposit ' .. amount .. '~s~ $')
	end
end)
...
AddEventHandler('jsfour-atm:take', function(amount)
	...
	if amount > accountMoney then
		...
		TriggerClientEvent('esx:showNotification', _source, 'you withdraw ' .. amount .. '~s~ $')
	end
end)
  ...
  function (result)
    if (result[1] ~= nil) then
      ...
      local senPlayer    = ESX.GetPlayerFromId(_source)
        ...
        function (result)
          if (result[1] ~= nil) then
            TriggerClientEvent('esx:showNotification', _source, 'tranfer  ' .. amount .. '~s~ $ to ' .. result[1].firstname .. ' ' .. result[1].lastname)
	...
	MySQL.Async.fetchAll('SELECT firstname, lastname FROM users WHERE identifier = @identifier', {['@identifier'] = ESX.GetPlayerFromId(_source).getIdentifier()},
            function (result)
              TriggerClientEvent('esx:showNotification', recPlayer.source, 'You got ' .. amount .. '~s~ $ tranfer from ' .. result[1].firstname .. ' ' .. result[1].lastname)
            end)
          end
        end)
      end
